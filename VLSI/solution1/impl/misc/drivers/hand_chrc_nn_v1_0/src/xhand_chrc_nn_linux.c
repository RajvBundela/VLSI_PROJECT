...
typedef struct {
    ...
    XHand_chrc_nn_uio_map maps[ MAX_UIO_MAPS ];
} XHand_chrc_nn_uio_info;

/***************** Variable Definitions **************************************/
static XHand_chrc_nn_uio_info uio_info;
...
static int uio_info_read_name(XHand_chrc_nn_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XHand_chrc_nn_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XHand_chrc_nn_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XHand_chrc_nn_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XHand_chrc_nn_Initialize(XHand_chrc_nn *InstancePtr, const char* InstanceName) {
	XHand_chrc_nn_uio_info *InfoPtr = &uio_info;
